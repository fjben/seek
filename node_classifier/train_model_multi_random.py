...
model_path = f'node_classifier/2_model_multi_random/{dataset}'
...
if aproximate_model:
    ...
    if not embeddings_for_all_entities:
        ...
        print('here')
        all_neigbours_plus_task_entities = all_neighbours[:]
        all_neigbours_plus_task_entities.extend(entities)
        all_neigbours_plus_task_entities = list(set(all_neigbours_plus_task_entities))
        neighbours_plus_task_entities_embeddings, walks_time, embeddings_fit_time = transformer_fit_transform_with_times(kg, all_neigbours_plus_task_entities)

        dic_emb_classes = dict()
        for neighbour, neighbour_embeddings in zip(all_neigbours_plus_task_entities, neighbours_plus_task_entities_embeddings):
            dic_emb_classes[neighbour] = neighbour_embeddings.tolist()

        all_embeddings = neighbours_plus_task_entities_embeddings
