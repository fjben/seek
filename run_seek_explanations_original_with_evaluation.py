...
def compute_performance_metrics(predicted_labels, list_labels):
    ...
    pr = metrics.precision_score(list_labels, predicted_labels)
    re = metrics.recall_score(list_labels, predicted_labels)
    return waf, pr, re


def compute_effectiveness_kelpie(path_dataset_labels, path_graph, path_label_classes, path_embedding_classes, target_pair, alg, path_file_model, path_explanations, n_embeddings=100, type='PPI'):

    pred_eva_necessary, y_eva_necessary = [], []
    pred_eva_sufficient, y_eva_sufficient = [], []
    ...
    with open(path_dataset_file, 'r') as dataset:
        for line in dataset:
            ...
            ent1, ent2, label = list_line[0], list_line[1], int(list_line[2])
            ...
            if int(pred_original) == label:
                ...
                pred_eva_necessary.append(int(pred_withoutnecessary))
                original_pred_eva_necessary.append(int(pred_original))
                y_eva_necessary.append(int(label))
            ...
            if int(pred_original) != label:
                ...
                pred_eva_sufficient.append(int(prediction_withsuff))
                original_pred_eva_sufficient.append(int(pred_original))
                y_eva_sufficient.append(int(label))
    ...
    with open(path_explanations + "ExplanationEfectivenessKelpie.txt", 'w') as file_output:
        ...
        file_output.write('OriginalNecessary\t' + str(original_waf_necc) + '\t' + str(original_pr_necc) + '\t' + str(
            original_re_necc) + '\n')
        ...
        file_output.write('OriginalSufficient\t' + str(original_waf_suf) + '\t' + str(original_pr_suf) + '\t' + str(original_re_suf) + '\n')
