...
def run_OWL2Vec(ontology_file, sampler, walker, walk_depth, n_walks, Mix_Type, embed_size, window_arg, iteration, negative_arg, min_count_arg, seed_arg, epoch, URI_Doc_arg = 'yes', Lit_Doc_arg = 'yes', Mix_Doc_arg ='yes', cache_dir='./cache', ontology_projection="no", pre_entity_file="", pre_axiom_file="", pre_annotation_file="",  pre_train_model=""):
    ...
    if URI_Doc_arg == 'yes':
        ...
        walks_ = get_rdf2vec_walks(dic_IC=, onto_file=ontology_file, sampler_type= sampler, n_walks=n_walks, walker_type=walker, walk_depth=walk_depth, classes=entities)
...
kg_file = "Data/PPI/go-basic-annots.owl" # kg file
path_output = "Embeddings/PPI" # folder where embeddings will be saved
embedding_file_path = path_output + '/Emb_owl2vec_with_skip-gram_wl.txt'

path_entity_file = "Data/PPI/Prots_v11(score950).txt" # file with entities for wich we wanna save the embeddings
entities =  [line.strip() for line in open(entities_file_path).readlines()]

embeddings = run_OWL2Vec(kg_file, sampler, walker, walk_depth, n_walks, Mix_Type, embed_size,
                                 window_arg, iteration, negative_arg, min_count_arg, seed_arg, epoch, URI_Doc_arg,
                                 Lit_Doc_arg, Mix_Doc_arg, cache_dir, ontology_projection, pre_entity_file,
                                 pre_axiom_file,
